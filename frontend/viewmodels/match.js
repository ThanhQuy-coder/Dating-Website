...
const chatProfiles = {
  "Hà My": {
    avatar: "https://i.pinimg.com/736x/e5/fc/a5/e5fca56c7194668b0e4c8a47f0f0b64a.jpg",
    status: "Active now",
    messages: [
  ...
  "Linh Tran": {
    avatar: "https://i.pinimg.com/736x/06/22/c7/0622c7b35454a3916004f7f3b56e52ad.jpg",
    status: "Online",
    messages: [
  ...
  "Thu Nguyen": {
    avatar: "https://i.pinimg.com/736x/38/fd/07/38fd076e6cf0e13ba30b44034ea5f2b9.jpg",
  ...
  "Mai Anh": {
    avatar: "https://i.pinimg.com/736x/51/4e/34/514e34ef914b74aff29eab6d6f51f077.jpg",
...
function loadNextCard() {
  ...
  if (cardDetails) cardDetails.textContent = `${profile.age} years old • ${profile.distance} away`
...
function addSearchModalEventListeners() {
  ...
  document.querySelectorAll(".distance-option").forEach((option) => {
    option.addEventListener("click", function () {
      document.querySelectorAll(".distance-option").forEach((opt) => opt.classList.remove("active"))
...
function resetSearchFilters() {
  ...
  document.querySelectorAll(".search-tag").forEach((tag) => tag.classList.remove("active"))
}

function applySearchFilters() {
  // Get selected filters
  const selectedInterests = Array.from(document.querySelectorAll(".search-tag.active")).map((tag) =>
    tag.textContent.trim(),
  )
  const selectedDistance = document.querySelector(".distance-option.active")?.textContent
...
function showSearchResults(interests, distance) {
  ...
  notification.innerHTML = `
    <div class="notification-content">
      ...
      <p>Found ${Math.floor(Math.random() * 50) + 10} potential matches within ${distance}</p>
    </div>
  `
...
function muteChat() {
  closeChatOptions()
  showNotification("Chat muted", "You won't receive notifications from this chat")
}

function archiveChat() {
  closeChatOptions()
  showNotification("Chat archived", "This chat has been moved to your archived conversations")
}

function blockUser() {
  closeChatOptions()
  if (confirm(`Are you sure you want to block ${currentChatUser}?`)) {
    showNotification("User blocked", `${currentChatUser} has been blocked and removed from your matches`)
  }
}
...
function reportUser() {
  ...
  if (confirm(`Report ${currentChatUser} for inappropriate behavior?`)) {
    showNotification("Report submitted", "Thank you for helping keep our community safe")
  }
}
  ...
  if (/iPad|iPhone|iPod/.test(navigator.userAgent)) {
    ...
    inputs.forEach((input) => {
      ...
      input.addEventListener("blur", () => {
        document
          .querySelector("meta[name=viewport]")
          .setAttribute("content", "width=device-width, initial-scale=1.0, maximum-scale=5.0, user-scalable=yes")
      })
    })
  }
}
...
// Help function
function showHelp() {
  showNotification("Help & Support", "Contact us at support@perfectmatch.com or visit our FAQ section")
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const defaultNavIcon = document.querySelector(".nav-icon[onclick=\"showTab('dating')\"]")
  const defaultBottomNavItem = document.querySelector(".bottom-nav-item[onclick=\"showTab('dating')\"]")
