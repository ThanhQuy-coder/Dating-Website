import { showProfileOther } from "../../../models/userModel.js";
import { setupHobbiesEditor, updateHobbiesDisplay } from "./hobbiesEditor.js";
import { setupImageUpload, reloadPhotos } from "./photos.js";

export async function loadProfileOther(id_profile_other) {
  const avatarSection = document.getElementsByClassName(
    "avatarSection-other-user"
  );
  const displayName = document.getElementsByClassName("displayName-other");
  ...
  try {
    const response = await showProfileOther(id_profile_other);
